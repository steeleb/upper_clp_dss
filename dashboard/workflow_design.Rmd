...
start_DT <- ymd_hms("2025-03-01 00:00:00", tz = "America/Denver")
...
all_data_raw <- c(hv_data, wet_data, contrail_data, log_data )
...
min_date <- as_date(min(all_data_raw$DT_round_MT, na.rm = TRUE))
max_date <- as_date(max(all_data_raw$DT_round_MT, na.rm = TRUE))

write_parquet(all_data_raw, here("data", "upper_clp_dss", "sensor", paste0("uclp_sensor_raw_", min_date, "_", max_date,".parquet")))
```
