...
flow_sites <- cdssr::get_telemetry_stations(water_district = 3 )%>% # Specify Poudre basin
  ...
  mutate(icon = ifelse(structure_type == "Stream Gage", "water", "triangle"))
    #structure_type %in% c("Ditch", "Other", "Measuring Point", "Reser")
my_icons <- awesomeIcons(icon = flow_sites$icon,
                         markerColor = "blue",
                         library = "glyphicon")
...
leaflet(site_loc) %>%
  ...
  addAwesomeMarkers(
    data = flow_sites,
    icon = ~my_icons[structure_type],
    popup = ~paste("Flow Site:", station_name), 
  )
  #setView(lng = -111.9, lat = 40.7, zoom = 8) # Adjust the view to center on the sites
```
...
# #plot
# new_WET_data %>%
#   filter(value %nin% invalid_values,!is.na(value), value >0 )%>%
...
# ggplot(contrail_data, aes(x = DT_round_MT, y = value, color = site)) +
#   geom_line() +
#   facet_wrap(~parameter, scales = "free_y")+
...
all_data_raw <- combined_data %>%
  ...
  mutate(mean = signif(mean, 3))
      ...
      p <- ggplot(all_data_raw%>% filter(!is.na(mean))%>%
                    filter(site %in% c("pman_fc", "pbd")), aes(x = DT_round_MT, y = mean, color = site)) +
        geom_path() +
        #theme_light()+
