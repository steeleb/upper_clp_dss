...
```{r}
z <- read_csv('data/upper_clp_dss/modeling/parsed_data_ROSS_virridy.csv') %>%
   #select(2:16)%>%
    rename(id = mw_id) %>%
...
z_norm <- z %>%
    dplyr::select( -Kjeldahl, -NO3, -TN, -Phosphorus, -DOC, -clean_sensor_datetime, -Turbidity, -Conductivity, -Depth, -Cl) %>%
...
ggplot(test, aes(x = TOC, y = toc_guess, color = collector)) + 
    geom_point()+
  geom_abline(slope = 1, intercept = 0, linetype = 'dashed') +
    labs(x = 'Measured TOC', y = 'Predicted TOC') +
    ROSS_theme

train$toc_guess <- predict(toc_mod, train_xg)

all <- bind_rows(dplyr::select(train, TOC, toc_guess, sensor_datetime) %>%
                     mutate(group = 'Train'),
                 dplyr::select(test, TOC, toc_guess, sensor_datetime) %>%
                     mutate(group = 'Test'))
