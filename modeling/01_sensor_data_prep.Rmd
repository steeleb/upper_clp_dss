...
site_levels <- c("salyer", "udall", "riverbend", "cottonwood","riverbend_virrridy", "cottonwood_virridy", "elc",
                 "archery", "archery_virridy" ,  "riverbluffs", "joei", "cbri", "chd", "pfal", "sfm", "pbd", "penn", "lbea", "springcreek", "boxcreek")
...
seasonal_thresholds_file <- "data/field_notes/qaqc/updated_seasonal_thresholds_2025.csv"
    ...
    future_map(
      function(data) {
        ...
        if (unique(data$parameter) %in% unique(season_thresholds$parameter)) {
          flagged_data <- flagged_data %>%
            data.table(.) %>%
#TODO: seasonal flags for upper clp sites not generated, Juan will be updating with additional sites shortly and will update existing filepath
...
# Bind all sensor data

```{r}
all_sensor_data <- bind_rows(all_data_2023, all_data_2024, all_data_2025)%>%
...
#
all_sensor_data%>%
  filter(parameter == "FDOM Fluorescence")%>%
  filter( site %in% c("pbd","udall", "cottonwood"))%>%
